...
import { CommitFormatter, PullRequestInfo } from '../git/gitService';
	...
	private async refresh(editor: TextEditor | undefined) {
		...
		if (
			...
			for (const l of lines) {
				...
				promises.push(Container.git.getPullRequestForCommit(state.commit.ref, remotes, { timeout: 100 }));
			}

			prs = new Map<string, PullRequestInfo | undefined>();
		...
		// if (prsTimedOut) {
		// 	setTimeout(() => this.refresh(editor), 250);
		// }
	}
